...
const HomePageMetaTag = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_SERVER_URL}/metatag`
      );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${process.env.REACT_APP_SERVER_URL}/metatag/${id}`);
  ...
  const handleOk = async () => {
    ...
    if (isCreatingNew) {
      ...
      if (editingItem) {
        try {
          await axios.put(
            `${process.env.REACT_APP_SERVER_URL}/metatag/${editingItem._id}`,
            editingItem
          );
          fetchData();
