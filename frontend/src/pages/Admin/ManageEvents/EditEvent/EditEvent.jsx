...
const EditEvent = () => {
    ...
    const handleEdit = async (id) => {
        ...
        if (titleImage) formDataToSend.append('titlepic', titleImage);
        if (detailImage) formDataToSend.append('detailpic', detailImage);

        try {
            const response = await axios.put(`${process.env.REACT_APP_SERVER_URL}/news/${id}`, formDataToSend, {
                headers: {
                ...
                <div className="flex gap-x-4">
                    <div className="w-full flex flex-col gap-y-2 pb-6">
                        ...
                        <input
                            ...
                            onChange={(e) => handleImageChange(e, setTitleImage)}
                            className="w-full"
                        />
                        {titleImage && (
                            <img
                    ...
                    <div className="w-full flex flex-col gap-y-2 pb-6">
                        ...
                        <input
                            type="file"
                            accept="image/jpeg, image/png"
                            onChange={(e) => handleImageChange(e, setDetailImage)}
                            className="w-full"
                        />
                        {detailImage && (
                            <img
